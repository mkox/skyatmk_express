...
exports.getActorLists = async function (body) {
  ...
  const [followedAtpActors, actors] = await Promise.all([
    ...
    AtpActor.aggregate([
      ...
      { $match : { 'actor.description': { $exists: true }} },
      { $sample: { size: sampleSize } }
    ])
  ]);

  var sortedActors = sortByInHowMuchSelectedFollowedUsers(actors, body.sort_by_in_how_much_selected_followed_actors)
  var actorsFinal = sortedActors.slice(0, parseInt(numberOfActors));
  var actorLists = {
...
function sortByInHowMuchSelectedFollowedUsers(actors, sort_by_in_how_much_selected_followed_actors) {
  if(sort_by_in_how_much_selected_followed_actors === 'on'){
    actors.sort(function(a, b){return b.followedDids.length - a.followedDids.length}); 
  }
  return actors;
}
